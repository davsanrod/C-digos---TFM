...
//Lectura y convertir a csv
	//2019 t2
cd "C:\Users\34654\Desktop\Big_Data\Master\TFM"
infile using "dic_epa.dct", using (md_EPA_2019T2.txt)
sum ciclo																		//compruebo, ciclo = 187 -> 2019
save "EPA_2019_T2", replace
clear all

	//2020 t2
infile using "dic_epa.dct", using (md_EPA_2020T2.txt)
sum ciclo																		//compruebo, ciclo = 191 -> 2019
save "EPA_2020_T2", replace
clear 

	//Merge muestra completa
use "EPA_2019_T2", clear
append using EPA_2020_T2
sum ciclo																		//compruebo, ciclo = 187 -191
save "EPA_muestraCompleta", replace

use "EPA_muestraCompleta"
...
gen inactivo = (aoi>=7 & aoi <=9)
	...
	*Clasificación por "ERTE"
gen si_ERTE = (rznotb == 11 & empleado==1)

	*Clasificación por "cobran el desempleo"
gen cobra = 1 if( parado==1 & ofemp==1)
replace cobra = 0 if (parado==1)
...
replace factorel= round(factorel/100)
	...
	*Aumenta la larga duracion?
tab parado if ciclo == 187
tab parado if ciclo == 191
tab larga_duracion if parado==1 & ciclo == 187
tab larga_duracion if parado==1 & ciclo == 191
...
save "EPA_muestraCompleta_varGeneradas", replace
